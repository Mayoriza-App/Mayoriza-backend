...
describe('CuentaContableService', () => {
  ...
  describe('findAll', () => {
    it('should return all accounts ordered by code', async () => {
      ...
      const result = await service.findAll('12345678-9');
      ...
      expect(mockPrismaService.cuentaEmpresa.findMany).toHaveBeenCalledWith({
        where: { empresaRut: '12345678-9', tipo: undefined },
        orderBy: { codigo: 'asc' },
      });
    });
    ...
    it('should apply tipo filter when provided', async () => {
      mockPrismaService.cuentaEmpresa.findMany.mockResolvedValue([mockCuenta]);
      await service.findAll('12345678-9', TipoCuenta.ACTIVO);
      expect(mockPrismaService.cuentaEmpresa.findMany).toHaveBeenCalledWith({
        where: { empresaRut: '12345678-9', tipo: TipoCuenta.ACTIVO },
        orderBy: { codigo: 'asc' },
      });
    });
  });
  ...
  describe('findOne', () => {
    it('should return a mapped DTO when account exists', async () => {
      ...
      const result = await service.findOne('12345678-9', mockCuenta.codigo);
    ...
    it('should throw NotFoundException when account does not exist', async () => {
      ...
      await expect(service.findOne('12345678-9', '9.9.99')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    it('should update and return the account on happy path', async () => {
      ...
      const result = await service.update('12345678-9', mockCuenta.codigo, {
        nombre: 'Caja Chica',
      });
    ...
    it('should throw NotFoundException when account does not exist', async () => {
      mockPrismaService.cuentaEmpresa.findUnique.mockResolvedValue(null);
      await expect(
        service.update('12345678-9', '9.9.99', { nombre: 'Inexistente' }),
  ...
  describe('remove', () => {
    it('should delete the account on happy path', async () => {
      ...
      await expect(service.remove('12345678-9', mockCuenta.codigo)).resolves.toBeUndefined();
      expect(mockPrismaService.cuentaEmpresa.delete).toHaveBeenCalledTimes(1);
    });

    it('should throw NotFoundException when account does not exist', async () => {
      mockPrismaService.cuentaEmpresa.findUnique.mockResolvedValue(null);
      await expect(service.remove('12345678-9', '9.9.99')).rejects.toThrow(NotFoundException);
    });
  });
});
