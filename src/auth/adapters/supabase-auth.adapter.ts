...
export class SupabaseAuthAdapter implements IAuthService {
  ...
  async validateToken(token: string): Promise<any> {
    if (!this.supabase) {
      this.logger.error(
        'Token validation attempted but Supabase Client is not initialized',
      );
    ...
    try {
      ...
      if (error || !data.user) {
        this.logger.warn(
          `Token validation failed: ${error?.message || 'No user found'}`,
        );
        throw new UnauthorizedException(
          'El token provisto es inválido o ha expirado',
        );
      }
